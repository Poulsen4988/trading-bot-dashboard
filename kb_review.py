"""
KB Review — analyserer knowledge/*.json for dubletter, stale items og lav-værdi indhold.

Kør: python kb_review.py
...
Output-struktur:
{
  ...
  "files": [
    {
      "path": "knowledge/NOVOb_xcse.json",
      "symbol": "NOVOb:xcse",
      "news_count": 42,
      "candidates": [
...
def main():
    ...
    for f in json_files:
        path = f.get("path") or f"knowledge/{f.get('name')}"
        kb = fetch_json(path)
        ...
        file_reports.append({
            "path": path,
            "symbol": kb.get("symbol") or path.replace("knowledge/", "").replace(".json", "").replace("_", ":"),
            "news_count": len(news),
